...
class ReceivedData:
    def __init__(self, sensor_data1, sensor_data2, sensor_data3, sensor_data4, sensor_data5):
...
class Uart(Node):
    Commands_to_send = CommandData()
    Commands_received = ReceivedData()
    ...
    def init_interface(self):
        ...
        self.sensor_data_publisher = self.create_publisher(FoilConsigne, '/controler_data', 10)
    ...
    def servo_angles_callback(self, msg):
        ...
        self.get_logger().info('Angles des servomoteurs - servoFoil: %f, servoGouvernail: %f, servoAileronLeft: %f, servoAileronRight: %f, Thruster: %f',
                                msg.servo_foil, msg.servo_gouvernail, msg.servo_aileron_left, msg.servo_aileron_right, msg.thruster)
    ...
    def time_callback(self):
        ...
        self.get_logger().info('Fonction time_callback appelée à intervalles réguliers.')
        ...
        if self.Commands_received is not None:
            self.get_logger().info(f'Données du capteur reçues: {self.Commands_received.__dict__}')
    ...
    def send_uart_data(self, msg):
        ...
        data_to_send = struct.pack('fffff', command_to_send.command1, command_to_send.command2,
                                   command_to_send.command3, command_to_send.command4, command_to_send.command5)
        
        # Envoyer les données via la liaison UART
        self.serial_port.write(data_to_send)

        # Afficher les données envoyées
        self.get_logger().info(f'Données de commandes envoyées: {command_to_send.__dict__}')
    ...
    def receive_sensor_data(self):
        ...
        if len(raw_data) == data_size:
            ...
            sensor_data = self.Commands_received(*struct.unpack('fffff', raw_data))
